inp = """Game 1: 1 green, 4 blue; 1 blue, 2 green, 1 red; 1 red, 1 green, 2 blue; 1 green, 1 red; 1 green; 1 green, 1 blue, 1 red
...
Game 100: 6 red, 4 green; 3 red, 2 blue, 9 green; 1 blue, 5 green, 14 red; 1 blue, 2 red, 2 green; 9 red, 1 blue, 14 green; 2 blue, 11 green, 8 red""".split("\n")
import numpy as np
ans = 0
allowed = {"blue": 14, "green": 13, "red": 12}
colourToIndex = {"blue": 0, "green": 1, "red": 2}
for i in inp:
    s = i.split(": ")
    gameId = int(s[0].split(" ")[1])
    info = s[1].split("; ")
    gameAllowed = True
    maxColours = [0, 0, 0]
    for j in info:
        cubeInfo = j.split(", ")
        for k in cubeInfo:
            cubeCount, cubeColor = int(k.split(" ")[0]), k.split(" ")[1]
            if maxColours[colourToIndex[cubeColor]]  < cubeCount:
                maxColours[colourToIndex[cubeColor]] = cubeCount
                
    ans += np.prod(maxColours)
print(ans)
